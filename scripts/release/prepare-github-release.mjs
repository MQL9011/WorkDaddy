...
export function releaseAssetNames(version, platforms = RELEASE_PLATFORMS) {
  const assets = {
    mac: [
      [`WorkDaddy-${version}-m-chip.dmg`, `WorkDaddy-${version}-arm64.dmg`],
      [`WorkDaddy-${version}-intel-chip.dmg`, `WorkDaddy-${version}-x64.dmg`],
      [`WorkDaddy-${version}-arm64.zip`],
      [`WorkDaddy-${version}-x64.zip`],
    ],
    linux: [
      [`WorkDaddy-${version}-linux-arm64.AppImage`],
      [`WorkDaddy-${version}-linux-arm64.deb`],
      [`WorkDaddy-${version}-linux-aarch64.pacman`],
      [`WorkDaddy-${version}-linux-aarch64.rpm`],
      [`WorkDaddy-${version}-linux-x86_64.AppImage`],
      [`WorkDaddy-${version}-linux-amd64.deb`],
      [`WorkDaddy-${version}-linux-x64.pacman`],
      [`WorkDaddy-${version}-linux-x86_64.rpm`],
    ],
    // electron-builder emits the MSIX-compatible package as .appx; publish the same bytes with the modern .msix extension.
    win: [[`WorkDaddy-${version}-win-x64.exe`], [`WorkDaddy-${version}-win-x64.zip`], [`WorkDaddy-${version}-win-x64.msix`, `WorkDaddy-${version}-win-x64.appx`]],
  }
