...
export async function verifyPackage({ mode, releaseDirectory = resolve('release'), env = process.env, arch = undefined }) {
  ...
  console.log(
    `Verified ${mode} DMG and ZIP for WorkDaddy ${packageJson.version}: archive integrity, contained application, exact native unpack allowlist and architectures, Electron fuses, and package size budgets (DMG payload: ${describeSizeMetrics(dmgMetrics)}; ZIP payload: ${describeSizeMetrics(zipMetrics)})${mode === 'public' ? ', signatures, notarization staples, and Gatekeeper' : ''}.`,
  )
}
