...
try {
  ...
  // CI runs the hermetic e2e suite as its own job (see .github/workflows/release.yml),
  // independent of packaging, for every platform — mirror that split here so local
  // packaging never depends on e2e; run `npm run test:e2e:hermetic` separately when needed.
  const verifyScript = skipVerify ? 'build:bundle' : 'release:verify:package'
