...
import type { AgentBrowserActivityEvent, AgentBrowserPointerEvent, AgentBrowserState, AppUpdateState, OmpInstallPhase, PrimeEventEnvelope, PrimeWorkApi, SessionChangeEvent, TerminalDataEvent, TerminalExitEvent } from '../../src/types/api'
...
const api: PrimeWorkApi = {
  app: {
    ...
    installOmp: () => ipcRenderer.invoke('app:install-omp'),
    onInstallOmpProgress: (callback) => subscribe<{ phase: OmpInstallPhase }>('app:install-omp-progress', (payload) => callback(payload.phase)),
  },
  updates: {
  ...
  projects: {
    ...
    listDirectory: (root, directory, harness) => ipcRenderer.invoke('projects:list-directory', root, directory, harness),
    ...
    regrant: (path, harness) => ipcRenderer.invoke('projects:regrant', path, harness),
  ...
  providers: {
    ...
    saveTokenccKey: (apiKey) => ipcRenderer.invoke('providers:save-tokencc-key', apiKey),
    tokenccStatus: () => ipcRenderer.invoke('providers:tokencc-status'),
    listModelProviders: () => ipcRenderer.invoke('providers:list-model-providers'),
    saveProvider: (draft) => ipcRenderer.invoke('providers:save-provider', draft),
    createCustomProvider: (draft) => ipcRenderer.invoke('providers:create-custom-provider', draft),
    deleteCustomProvider: (providerId) => ipcRenderer.invoke('providers:delete-custom-provider', providerId),
    discoverModels: (input) => ipcRenderer.invoke('providers:discover-models', input),
  },
  terminal: {
  ...
  plugins: {
    ...
    readSkillDocument: (path, harness) => ipcRenderer.invoke('plugins:read-skill-document', path, harness),
  },
  settings: {
