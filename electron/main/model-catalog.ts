 ...
 *
 * Auth mutations are intentionally excluded: OMP authentication is owned by
 * the omp CLI itself, not the desktop app.
 */
...
export function catalogLimitWarnings(
  ...
  if (omittedProviders > 0) {
    warnings.push(`${harnessLabel} returned ${count(result.totalProviderCount)} valid providers; WorkDaddy retained ${count(result.providers.length)} sorted by name (${count(omittedProviders)} omitted by the provider limit).`)
  }
  if (omittedModels > 0) {
