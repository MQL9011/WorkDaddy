...
export class MacBackgroundController {
  ...
  private ensureTray(): void {
    ...
    tray.setToolTip('WorkDaddy')
    tray.setContextMenu(Menu.buildFromTemplate([
      { label: 'Open WorkDaddy', icon: menuIcon('<rect width="18" height="18" x="3" y="3" rx="2"/><path d="M9 3v18"/>'), click: () => this.open() },
      { type: 'separator' },
  ...
  private syncLoginItem(openAtLogin: boolean): void {
    ...
    try {
      ...
      console.error(`WorkDaddy could not update its login item: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
