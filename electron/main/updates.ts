...
export function manualUpdateNotification(state: AppUpdateState): ManualUpdateNotification {
  if (state.phase === 'error') return {
    type: 'error',
    message: 'WorkDaddy Update Check Failed',
  ...
  return {
    type: 'info',
    message: available ? 'WorkDaddy Update Available' : 'No WorkDaddy Update Available',
  }
}
...
export function createManualUpdateCheck(
  ...
  return () => {
    ...
    void check()
      ...
      .catch((error) => notify({ type: 'error', message: 'WorkDaddy Update Check Failed', detail: errorMessage(error) }))
