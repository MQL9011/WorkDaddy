...
const authorizeSessionPath: SessionPathAuthorizer = (root, path) => isPathWithin(root, path) && path.endsWith('.jsonl')

export interface SessionServiceOptions {
  /** Harness stamped on every record and change event this service emits; defaults to 'omp'. */
  harness?: HarnessId
  /** Canonical-or-lexical session root; defaults to the OMP agent session directory. */
  sessionRoot?: string
  catalogIo?: SessionCatalogIo
  /** Pre-I/O discovery ordering for a harness's file-name scheme. */
...
export class SessionService {
  ...
  constructor(
    ...
    this.sessionRoot = options.sessionRoot ?? join(homedir(), '.omp', 'agent', 'sessions')
    ...
    this.isSessionPathAuthorized = options.isSessionPathAuthorized ?? authorizeSessionPath
  ...
  async rename(filePath: unknown, title: unknown): Promise<boolean> {
    ...
    const result = await runProcess(agentPath, ['rename', metadata.id, safeTitle, '--json'], { timeoutMs: 30_000 })
