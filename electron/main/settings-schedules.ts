    ...
    const validators: { [K in keyof AppSettings]: (value: unknown) => AppSettings[K] } = {
      ...
      enabledHarnesses: (value) => {
        ...
        const enabled = [...new Set(value.map((entry, index) => {
          if (entry !== 'omp') throw new TypeError(`enabledHarnesses[${index}] is invalid`)
          return entry
        }))]
      ...
      activeHarness: (value) => {
        if (value !== 'omp') throw new TypeError('Invalid harness')
        return value
      },
      ...
      onboardingCompleted: (value) => requireBoolean(value, 'onboardingCompleted'),
