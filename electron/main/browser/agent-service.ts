...
function logNavigationFailure(url: string, error: unknown): void {
  ...
  console.warn(`WorkDaddy browser could not load ${url}: ${message}`)
}
...
export class AgentBrowserService {
  ...
  navigateTab(tabIdValue: unknown, actionValue: unknown, urlValue?: unknown): boolean {
    ...
    if (action === 'back') { if (guest.canGoBack()) guest.goBack() }
      ...
      if (!isAllowedTabUrl(url)) throw new Error('Only credential-free http(s) URLs can be opened in the WorkDaddy browser')
  ...
  async openTab(sessionKey: string, params: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if (this.tabs.size >= MAX_TABS_TOTAL) throw new Error('WorkDaddy has too many open agent browser tabs. Close unused tabs first.')
    const tab: TabState = {
  ...
  private requireOpen(): void {
    if (this.closed) throw new Error('WorkDaddy is shutting down')
  }
    ...
    await new Promise<void>((resolveAttach, reject) => {
      const waiter = {
        ...
        timer: setTimeout(() => {
          ...
          reject(new Error('The WorkDaddy browser pane did not attach this tab. Make sure the WorkDaddy window is open.'))
