...
import type { HarnessId, PluginCatalog, PluginWarning, SkillDocument, SkillDocumentFile, SkillRecord } from '../../../src/types/api'
import {
...
async function directoryHasSkillFile(path: string): Promise<boolean> {
  try {
    const directory = await opendir(path)
    for await (const entry of directory) {
      if (entry.isFile() && entry.name === 'SKILL.md') return true
    }
  } catch { /* unreadable, or not a directory */ }
  return false
}

const MAX_SKILL_DOCUMENT_FILES = 200

/**
 * Reads one skill's full markdown document and lists its sibling files for
 * the detail view. The caller must have already authorized `safePath` (e.g.
 * via `PluginService.authorizeReveal`) — this function performs no access
 * control of its own.
 */
export async function readSkillDocument(safePath: string): Promise<SkillDocument> {
  const { content, truncated } = await readAtMost(safePath, MAX_METADATA_BYTES)
  const frontmatterEnd = content.indexOf('\n---', 3)
  const frontmatter = content.startsWith('---') ? content.slice(3, frontmatterEnd >= 0 ? frontmatterEnd : 0) : ''
  const body = content.startsWith('---') && frontmatterEnd >= 0 ? content.slice(frontmatterEnd + 4).replace(/^\r?\n/, '') : content
  const baseDir = dirname(safePath)

  const files: SkillDocumentFile[] = []
  try {
    const directory = await opendir(baseDir)
    for await (const entry of directory) {
      if (files.length >= MAX_SKILL_DOCUMENT_FILES) break
      if (entry.name.startsWith('.') || entry.name === 'SKILL.md' || entry.isSymbolicLink()) continue
      const isDirectory = entry.isDirectory()
      // A subdirectory's own SKILL.md is invisible to omp's non-recursive
      // discovery (only <root>/skills/<name>/SKILL.md is scanned); surfacing
      // it here is the only place the UI can catch that mistake.
      const hasNestedSkill = isDirectory ? await directoryHasSkillFile(join(baseDir, entry.name)) : false
      files.push({ name: entry.name, isDirectory, hasNestedSkill })
    }
  } catch { /* directory changed while it was being traversed */ }
  files.sort((left, right) => (left.isDirectory === right.isDirectory ? left.name.localeCompare(right.name) : left.isDirectory ? -1 : 1))

  return {
    frontmatter: {
      name: scalar(frontmatter, 'name'),
      description: scalar(frontmatter, 'description'),
      globs: scalar(frontmatter, 'globs'),
      alwaysApply: scalar(frontmatter, 'alwaysApply') ?? scalar(frontmatter, 'always_apply'),
      disableModelInvocation: scalar(frontmatter, 'disable-model-invocation'),
    },
    body,
    truncated,
    baseDir,
...
async function buildCandidateRecords(candidates: Candidate[], safeProjectPath?: string, harness: HarnessId = 'omp'): Promise<SkillRecord[]> {
...
function addMcpSettingsMetadata(
  ...
  if (!isRecord(settings.mcpServers)) return
  ...
  for (const [name, raw] of entries.slice(0, MAX_MCP_DISCOVERY_RECORDS)) {
    ...
    if (definition && isNetworkMcpDefinition(definition)) {
      ...
      output.push({
        ...
        description: `${transport} MCP server at ${origin}. Managed outside WorkDaddy.`,
    ...
    } else if (definition && definition.type === 'stdio' && typeof definition.command === 'string') {
      const command = basename(definition.command).slice(0, 120)
      const availability = !isAppManageableLocalMcpKey(name)
        ? { available: false as const, detail: LOCAL_MCP_STATE_UNAVAILABLE_DETAIL }
        : undefined
      output.push({
        id: idFor('mcp', location, name), name: displayName,
        description: `Local stdio MCP server (${command})`,
...
export async function discoverPlugins(agentDir: string, safeProjectPath: string | undefined, harness: HarnessId = 'omp'): Promise<PluginCatalog> {
  ...
  await collectDirectory(resolve(agentDir, '..', 'skills'), 'skill', 'user', candidates, budget, { skillRoot: true })
  ...
  await collectOmpPluginPackages(resolve(agentDir, '..', 'plugins'), 'user', safeProjectPath, candidates, ompPackageRecords, budget)
  ...
  if (safeProjectPath && isAbsolute(safeProjectPath) && await pathExists(safeProjectPath)) {
    ...
    // A project can point back at the same agent settings and shared skills
  ...
  await addPackageSettingsMetadata(globalSettings, join(agentDir, 'settings.json'), 'user', result)
  await addPackageSettingsMetadata(projectSettings, safeProjectPath ? join(safeProjectPath, ...PROJECT_AGENT_SEGMENTS[harness], 'settings.json') : '', 'project', result)
  // OMP reads MCP definitions from its own mcp.json, not settings.json.
  ...
  addMcpSettingsMetadata(globalMcp.settings, globalMcpPath, 'user', mcpRecords, warnings, harness)
  if (safeProjectPath) {
    ...
    addMcpSettingsMetadata(projectMcp.settings, projectMcpPath, 'project', mcpRecords, warnings, harness)
  }
