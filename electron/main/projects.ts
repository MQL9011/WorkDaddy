...
/** Cap for the recursive, whole-tree scan backing the file-tree search box. */
const MAX_RECURSIVE_ENTRIES = 50_000
/** Cap for a single directory's immediate children, backing lazy tree expansion. */
 ...
 * its own harness. Dismissed inferred-project paths remain shared, matching
...
export class ProjectService {
  ...
  constructor(
    ...
    private readonly harness: HarnessId = 'omp',
  ...
  /** Reads one directory's children, applying the ignore/symlink/type rules shared by every listing mode. */
  private async readVisibleChildren(directory: string): Promise<{ children: Dirent[]; readable: boolean }> {
    let children: Dirent[]
    try {
    ...
    const visible = children.filter((child) => {
      ...
      if (child.isDirectory() && IGNORED_DIRECTORIES.has(child.name)) return false
    ...
    return { children: visible, readable: true }
  }

  private relativeEntryPath(root: string, absolutePath: string): string {
    const relativePath = relative(root, absolutePath)
    return process.platform === 'win32' ? relativePath.split('\\').join('/') : relativePath
  }

  /** Full recursive scan of the project tree, used to back the file-tree search box. */
  async listFiles(rootValue: unknown): Promise<ProjectFileListing> {
    ...
    const visit = async (directory: string): Promise<void> => {
      if (entries.length >= MAX_RECURSIVE_ENTRIES) return
      const { children, readable } = await this.readVisibleChildren(directory)
      if (!readable) {
        skipped += 1
        return
      }
      for (const child of children) {
        if (entries.length >= MAX_RECURSIVE_ENTRIES) break
        const absolutePath = resolve(directory, child.name)
        entries.push({ path: this.relativeEntryPath(root, absolutePath), type: child.isDirectory() ? 'directory' : 'file' })
        if (child.isDirectory()) await visit(absolutePath)
      }
    }

    await visit(root)
    return { entries, skipped, truncated: entries.length >= MAX_RECURSIVE_ENTRIES }
  }

  /** Immediate children of a single directory, used to expand one node of the file tree on demand. */
  async listDirectory(rootValue: unknown, directoryValue: unknown): Promise<ProjectFileListing> {
    const root = await this.authorizeCwd(rootValue as string)
    const directory = await this.authorizeCwd(directoryValue as string)
    if (!isPathWithin(root, directory)) throw new TypeError('directory must be inside root')

    const { children, readable } = await this.readVisibleChildren(directory)
    if (!readable) return { entries: [], skipped: 1, truncated: false }

    const entries: ProjectFileEntry[] = children.slice(0, MAX_DIRECTORY_ENTRIES).map((child) => ({
      path: this.relativeEntryPath(root, resolve(directory, child.name)),
      type: child.isDirectory() ? 'directory' : 'file',
    }))
    return { entries, skipped: 0, truncated: children.length > entries.length }
  }
