...
describe('NoHarnessPrompt', () => {
  ...
  it('explains recovery options and opens Harness settings from the primary action', async () => {
    ...
    expect(document.querySelector('[role="dialog"]')?.textContent).toContain('OMP not detected')
    expect(document.querySelector('[role="dialog"]')?.textContent).toContain('OMP (oh-my-pi)')
