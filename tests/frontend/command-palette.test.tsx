...
const harnessState = vi.hoisted(() => ({ activeHarness: 'omp' as HarnessId }))
...
vi.mock('../../src/hooks/useAppSettings', async (importOriginal) => {
  ...
  return {
    ...
    useAppSettings(options: Parameters<typeof actual.useAppSettings>[0]) {
      ...
      return { ...state, settings: { ...state.settings, activeHarness: harnessState.activeHarness, locale: 'en' } }
    },
  }
})
...
const expectedOrder = [
  ...
  'Toggle sidebar',
  'Open Settings',
  // PoC-only vault decryption demo; see CommandPalette.tsx.
  'Vault demo: code review',
]

describe.each<HarnessId>(['omp'])('command palette for %s', (harness) => {
  it('keeps Activity searchable and activatable by keyboard and pointer', async () => {
    ...
    await search(input, 'activity')
    options = commandOptions(dialog)
    expect(options.map((option) => option.querySelector('strong')?.textContent)).toEqual(['Open Activity'])
    expect(options[0].getAttribute('aria-selected')).toBe('true')
    ...
    await waitFor(() => [...document.body.querySelectorAll('h1')].find((heading) => heading.textContent === 'Activity'))
    ...
    await waitFor(() => document.activeElement === input ? input : null)
    const activity = commandOptions(dialog).find((option) => option.textContent?.includes('Open Activity'))!
    await act(async () => {
      activity.dispatchEvent(new MouseEvent('mouseover', { bubbles: true }))
    })
