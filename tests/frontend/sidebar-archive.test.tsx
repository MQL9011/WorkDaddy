...
describe('sidebar project context menu', () => {
  it('requires confirmation before downloading and restarting an available update', async () => {
    ...
    expect(update?.getAttribute('title')).toBe('Download and restart WorkDaddy 0.2.0')
    expect(update?.querySelector('.lucide-download')).not.toBeNull()
    await press(update!)
    expect(onUpdateAction).not.toHaveBeenCalled()
    expect(document.body.querySelector('[role="dialog"]')?.textContent).toContain('Download and Restart WorkDaddy?')
  ...
  it('hides the update control next to Settings when no update is available', async () => {
    const onUpdateAction = vi.fn()
    await act(async () => {
      root.render(
        <Sidebar
