...
describe('i18n', () => {
  ...
  it('interpolates values and selects plural forms', () => {
    ...
    // common.reload is fully translated (used on the top-level fatal-error screen in main.tsx,
    // which renders outside the I18nProvider tree and calls translate() directly).
    expect(translate('zh-CN', 'common.reload')).toBe('重新加载 WorkDaddy')
    expect(translate('en', 'common.reload')).toBe('Reload WorkDaddy')
  })
})
