...
// Lets pending IPC-mock promises (and the state updates chained after them) settle.
async function flush() {
  await Promise.resolve()
  await Promise.resolve()
}

function findTreeItem(el: HTMLElement, name: string): HTMLButtonElement {
  const match = [...el.querySelectorAll<HTMLButtonElement>('.file-tree__item')].find(
    (item) => item.querySelector('.file-tree__name')?.textContent === name,
  )
  if (!match) throw new Error(`no tree item named ${name}`)
  return match
}

// Simulates the backend's non-recursive listDirectory by slicing a flat, fully-recursive
// entries fixture down to the immediate children of whichever directory is requested.
function makeListDirectory(entries: ProjectFileEntry[]) {
  return vi.fn(async (root: string, directory: string) => {
    const prefix = directory === root ? '' : `${directory.slice(root.length + 1)}/`
    const seen = new Set<string>()
    const children: ProjectFileEntry[] = []
    ...
    return { entries: children, skipped: 0, truncated: false }
  })
}
...
describe('FilesPanel component', () => {
  ...
  it('renders files in a hierarchical tree, lazily fetching each folder on click', async () => {
    const listDirectory = makeListDirectory(sampleEntries)
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory } },
    })
    ...
    expect(listDirectory).toHaveBeenCalledWith('/workspace/test-project', '/workspace/test-project', 'omp')
    ...
    // Initially collapsed by default: top-level files and folders only, none fetched yet
    ...
    // Clicking 'src' fetches and expands it -> 'components' appears
    await act(async () => {
      srcButton.click()
      await flush()
    })
    expect(listDirectory).toHaveBeenCalledWith('/workspace/test-project', '/workspace/test-project/src', 'omp')
    ...
    // Clicking 'components' fetches and expands it -> chat, layout, terminal appear
    await act(async () => {
    ...
    // Clicking 'chat' fetches and expands it -> AgentProgress.tsx, ChatPanel.tsx appear
    await act(async () => {
      chatButton.click()
      await flush()
    })
  ...
  it('does not refetch a folder that is collapsed and re-expanded', async () => {
    const listDirectory = makeListDirectory(sampleEntries)
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory } },
    ...
    await act(async () => {
      findTreeItem(container, 'src').click() // expand + fetch
      await flush()
    })
    expect(listDirectory).toHaveBeenCalledTimes(2) // root + src

    act(() => { findTreeItem(container, 'src').click() }) // collapse, no fetch
    act(() => { findTreeItem(container, 'src').click() }) // re-expand, already loaded
    expect(listDirectory).toHaveBeenCalledTimes(2)

    expect([...container.querySelectorAll('.file-tree__name')].map((el) => el.textContent)).toEqual([
      'src',
      'components',
      'package.json',
      'README.md',
  ...
  it('displays git modification status for changed files as their folders are expanded', async () => {
    const listDirectory = makeListDirectory(sampleEntries)
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory } },
    ...
    // Drill down to the nested changed file, fetching each level on the way
    await act(async () => {
      findTreeItem(container, 'src').click()
      await flush()
    })
    await act(async () => {
      findTreeItem(container, 'components').click()
      await flush()
    })
    await act(async () => {
      findTreeItem(container, 'chat').click()
      await flush()
    })
  ...
  it('searches the full project tree via a separate recursive fetch, not just loaded folders', async () => {
    const listDirectory = makeListDirectory(sampleEntries)
    const listFiles = vi.fn(async () => ({ entries: sampleEntries, skipped: 0, truncated: false }))
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory, listFiles } },
    })
    ...
    expect(listFiles).toHaveBeenCalledWith('/workspace/test-project', 'omp')
    const filteredNames = [
    ...
    // Cached full listing is reused for a second search - no second fetch.
    await enter(searchInput, 'ChatPanel')
    await act(async () => {
      await flush()
    })
  ...
  it('flags incomplete search results when the recursive scan is truncated', async () => {
    const listDirectory = makeListDirectory(sampleEntries)
    const listFiles = vi.fn(async () => ({ entries: sampleEntries, skipped: 0, truncated: true }))
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory, listFiles } },
    })
    ...
    expect(container.textContent).toContain(
      'Search results are incomplete — this project has too many files to search in full.',
    )
  })
  ...
  it('shows an inline error and leaves the tree intact when expanding one folder fails', async () => {
    const listDirectory = vi.fn(async (_root: string, directory: string) => {
      if (directory.endsWith('/src')) throw new Error('Permission denied')
      return { entries: sampleEntries.filter((entry) => !entry.path.includes('/')), skipped: 0, truncated: false }
    })
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory } },
    ...
    // The failed folder collapses back and shows an inline error; the rest of the tree survives.
    expect(container.querySelector('.file-tree__status--deleted')?.textContent).toBe('!')
    expect(findTreeItem(container, 'src').getAttribute('aria-expanded')).toBe('false')
    expect([...container.querySelectorAll('.file-tree__name')].map((el) => el.textContent)).toEqual([
      'src',
      'package.json',
      'README.md',
  ...
  it('handles collapse all and expand all toolbar actions, fetching newly-known folders', async () => {
    const listDirectory = makeListDirectory(sampleEntries)
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory } },
    ...
    // "Expand all" expands and fetches every currently-known directory ('src' is the only
    // one yet known); newly-revealed 'components' stays collapsed until expanded itself.
    const expandedNames = [
      ...container.querySelectorAll<HTMLButtonElement>('.file-tree__item'),
    ].map((el) => el.querySelector('.file-tree__name')?.textContent)
    expect(expandedNames).toEqual(['src', 'components', 'package.json', 'README.md'])
  ...
  it('renders multi-root project folders in component', async () => {
    ...
    const listDirectory = vi.fn(async (root: string) => {
      if (root.includes('frontend')) {
        return { entries: [{ path: 'App.tsx', type: 'file' as const }], skipped: 0, truncated: false }
      }
      return { entries: [{ path: 'server.ts', type: 'file' as const }], skipped: 0, truncated: false }
    })
    Object.defineProperty(window, 'prime', {
      configurable: true,
      value: { projects: { listDirectory } },
  ...
  it('renders error banner if the initial listDirectory call fails', async () => {
    const listDirectory = vi.fn(async () => {
