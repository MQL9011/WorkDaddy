...
import type { SkillRecord, TranscriptMessage } from '../../src/types/api'
...
vi.mock('../../src/components/ui', async () => {
  ...
  return { ...actual, OmpMark: ({ size = 24 }: { size?: number }) => element('span', { className: 'omp-mark', style: { width: size, height: size } }) }
})

const git = { isRepo: false, files: [] }
const noop = () => undefined

function render(messages: TranscriptMessage[], active = false, showTools = true, skills: SkillRecord[] = []): string {
...
describe('transcript rendering', () => {
  ...
  it('shows a fresh working placeholder instead of reopening the prior assistant after a user tail', () => {
    ...
    expect(html).toContain('OMP is working')
  })
  ...
  it('renders system failures as compact expandable activity instead of red boxes', () => {
    ...
    expect(html).toContain('OMP message')
  ...
  it('renders compaction activity and completion separately from system failures', () => {
    ...
    expect(html).toContain('aria-expanded="false"')
    expect(html).not.toContain('OMP message')
  })
  ...
  it('flags a completed turn that touched a known skill file with a best-effort hit card', () => {
    const skills: SkillRecord[] = [{
      id: 'reviewer-skill', name: 'reviewer', description: '', kind: 'skill', location: 'user', enabled: true,
      path: '/home/user/.omp/skills/reviewer/SKILL.md',
    }]
    const html = render([{
      id: 'used-skill',
      role: 'assistant',
      ...
      completedAt: 2_000,
      parts: [
        { type: 'toolCall', id: 'tool-1', name: 'read', args: { path: '/home/user/.omp/skills/reviewer/checklist.md' } },
        { type: 'toolResult', name: 'read', text: 'done' },
        { type: 'text', text: 'Applied the checklist.' },
      ],
    }], false, false, skills)

    expect(html).toContain('skill-hit-card')
    expect(html).toContain('Possibly used reviewer')
  })

  it('does not render a skill-hit card while a turn is still streaming or when nothing matched', () => {
    const skills: SkillRecord[] = [{
      id: 'reviewer-skill', name: 'reviewer', description: '', kind: 'skill', location: 'user', enabled: true,
      path: '/home/user/.omp/skills/reviewer/SKILL.md',
    }]
    const streaming = render([{
      ...
      timestamp: 1_000,
      streaming: true,
      parts: [{ type: 'toolCall', id: 'tool-1', name: 'read', args: { path: '/home/user/.omp/skills/reviewer/checklist.md' } }],
    }], true, false, skills)
    expect(streaming).not.toContain('skill-hit-card')
    ...
    const noMatch = render([{
      ...
      timestamp: 1_000,
      completedAt: 2_000,
      parts: [{ type: 'toolCall', id: 'tool-1', name: 'read', args: { path: '/tmp/unrelated.txt' } }],
    }], false, false, skills)
    expect(noMatch).not.toContain('skill-hit-card')
  })
})
