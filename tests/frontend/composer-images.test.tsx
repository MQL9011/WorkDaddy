...
describe('Composer memoization', () => {
  ...
  it('does not re-render when a streaming parent re-renders with identical props', () => {
    ...
    expect(menu?.querySelectorAll('.menu-select__group')).toHaveLength(2) // ungrouped "auto" + the one provider
    expect(menu?.querySelectorAll('[role="menuitemradio"]')).toHaveLength(2) // "auto" + the one model
  })
})
