...
test.describe('Prime Work desktop smoke', () => {
  ...
  test.beforeEach(async ({}, testInfo) => {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        const environment = hermeticEnvironment(fixture.home, fixture.executable, fixture.ompExecutable, fixture.piExecutable, liveInstall || noHarnesses)
  ...
  test('loads the sandboxed preload bridge and hermetic service data', async () => {
    ...
    expect(bridge.groups).toEqual(['agent', 'app', 'browser', 'git', 'plugins', 'projects', 'providers', 'sessions', 'settings', 'terminal', 'updates'])
    ...
    expect(updateMenu.parent).toBe(updateMenu.platform === 'darwin' ? 'WorkDaddy' : 'Help')
...
`)
  ...
  test('navigates all primary workspace pages and command palette', async () => {
    for (const destination of ['Projects', 'Activity', 'Capabilities']) {
      ...
      if (destination === 'Capabilities') {
        ...
        await expect(addDialog.getByText(/Not every third-party package, plugin, or extension will work in WorkDaddy/)).toBeVisible()
        const addMcp = addDialog.getByRole('button', { name: /Add MCP/ })
        await expect(addMcp).toBeDisabled()
        await expect(addMcp).toContainText('Prime MCP setup is managed outside WorkDaddy')
  ...
  test('Command-Q backgrounds the window and menu-bar Open restores it', async () => {
    ...
    await app!.evaluate(({ Menu }) => {
      ...
      Menu.buildFromTemplate = ((template) => {
        ...
        if (items.map((item) => item.label ?? item.type).join('|') === 'Open WorkDaddy|separator|Settings...|Quit') {
          scope.__trayOpen = items.find((item) => item.label === 'Open WorkDaddy')?.click
        }
