...
function createUnpackedFixture(architectures = new Set(['arm64'])) {
  ...
  const relativePaths = expectedUnpackedNativeLayout(architectures).files
...
function writeMacReleaseFixture(directory: string, { omit }: { omit?: string } = {}) {
  ...
  for (const [index, name] of expectedDownloadedReleaseAssets('0.2.0', ['mac']).entries()) {
    ...
    const content = name.endsWith('.yml')
      ? 'version: 0.2.0\nfiles:\n  - url: WorkDaddy-0.2.0-x64.zip\n    sha512: x64-digest\n    size: 42\npath: WorkDaddy-0.2.0-x64.zip\nsha512: x64-digest\n'
      : `asset ${index}`
...
function createPinnedNpmArtifactFixture(contents = Buffer.from('verified npm archive fixture')) {
  const temporaryDirectory = mkdtempSync(join(tmpdir(), 'workdaddy npm artifact & '))
...
describe('release preflight', () => {
  ...
  test('probes npm through its JavaScript CLI and both release entry points reject an old npm', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-npm-version-'))
  ...
  test('exact npm CLI verification ignores a stale lifecycle npm_execpath', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy exact npm & version-'))
  ...
  test('executes the installed npm CLI and selects its shim for the next workflow step', () => {
    const fixture = createPinnedNpmArtifactFixture()
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy stale npm & bootstrap-'))
...
`,
  ...
  test('installs the checked-in npm archive into a fresh prefix without network access', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy real npm bootstrap & '))
  ...
  test('persists only a validated npm shim directory for the next GitHub Actions step', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-github-path-'))
    const githubPath = join(directory, 'github-path')
  ...
  test('keeps contributor instructions aligned with the enforced engines', () => {
    ...
    expect(readme).toContain(`Node.js ${toolchain.node} 或更新，以及 npm ${toolchain.npm} 或更新`)
  ...
  test('passes the expected Authenticode signer to Windows packaging', () => {
    const workflow = readFileSync('.github/workflows/release.yml', 'utf8')
    expect(workflow).toContain('WORKDADDY_WINDOWS_CERT_SUBJECT: ${{ vars.WORKDADDY_WINDOWS_CERT_SUBJECT }}')
    expect(workflow).toContain('WORKDADDY_WINDOWS_CERT_THUMBPRINT: ${{ vars.WORKDADDY_WINDOWS_CERT_THUMBPRINT }}')
    const security = readFileSync('docs/security.md', 'utf8')
    expect(security).toContain('`WORKDADDY_WINDOWS_CERT_SUBJECT`')
    expect(security).toContain('`WORKDADDY_WINDOWS_CERT_THUMBPRINT`')
  ...
  test('ships both mac architectures as separate builds from native-arch runners', () => {
    ...
    // target (arm64 on macos-15, x64 on macos-15-intel) so node-pty never
    // cross-compiles, and one leg's failure never cancels the other's build.
    ...
    expect(releaseWorkflow).toContain('name: workdaddy-public-macos-${{ matrix.arch }}')
  ...
  test('ships both Linux architectures as separately labeled native builds', () => {
    ...
    expect(releaseWorkflow).toContain('name: workdaddy-public-linux-${{ matrix.arch }}')
...
describe('GitHub Release publication', () => {
  test('selects an exact enabled platform set', () => {
    ...
    expect(expectedGitHubReleaseAssets('0.2.0', ['mac', 'linux'])).not.toContain('WorkDaddy-0.2.0-win-x64.exe')
    expect(expectedGitHubReleaseAssets('0.2.0', ['mac'])).toEqual([
      'WorkDaddy-0.2.0-arm64.zip',
      'WorkDaddy-0.2.0-intel-chip.dmg',
      'WorkDaddy-0.2.0-m-chip.dmg',
      'WorkDaddy-0.2.0-x64.zip',
      'latest-mac.yml',
    ])
    expect(expectedDownloadedReleaseAssets('0.2.0', ['mac'])).toEqual([
      'WorkDaddy-0.2.0-arm64.zip',
      'WorkDaddy-0.2.0-x64.dmg',
      'WorkDaddy-0.2.0-arm64.dmg',
      'WorkDaddy-0.2.0-x64.zip',
      'latest-mac.yml',
    ])
    expect(expectedGitHubReleaseAssets('0.2.0', ['linux'])).toEqual([
      'WorkDaddy-0.2.0-linux-aarch64.pacman',
      'WorkDaddy-0.2.0-linux-aarch64.rpm',
      'WorkDaddy-0.2.0-linux-amd64.deb',
      'WorkDaddy-0.2.0-linux-arm64.AppImage',
      'WorkDaddy-0.2.0-linux-arm64.deb',
      'WorkDaddy-0.2.0-linux-x64.pacman',
      'WorkDaddy-0.2.0-linux-x86_64.AppImage',
      'WorkDaddy-0.2.0-linux-x86_64.rpm',
  ...
  test('keeps every release asset name in exact correspondence with electron-builder artifacts', () => {
    ...
    expect([...names].filter(([published, downloaded]) => published !== downloaded)).toEqual([
      ['WorkDaddy-0.2.0-intel-chip.dmg', 'WorkDaddy-0.2.0-x64.dmg'],
      ['WorkDaddy-0.2.0-m-chip.dmg', 'WorkDaddy-0.2.0-arm64.dmg'],
      ['WorkDaddy-0.2.0-win-x64.msix', 'WorkDaddy-0.2.0-win-x64.appx'],
    ])
  })
  ...
  test('reports the published name of a missing artifact through the rename map', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-github-release-missing-'))
    try {
      const { projectDirectory, inputDirectory } = writeMacReleaseFixture(directory, { omit: 'WorkDaddy-0.2.0-arm64.dmg' })
      ...
      await expect(prepareGitHubRelease({ inputDirectory, outputDirectory: join(directory, 'out'), projectDirectory, platforms: ['mac'], tag: 'v0.2.0' })).rejects.toThrow(
        /incomplete; missing WorkDaddy-0\.2\.0-m-chip\.dmg$/,
      )
    } finally {
      rmSync(directory, { recursive: true, force: true })
    }
  ...
  test('takes legacy updater fields from the primary architecture manifest, not the first sorted path', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-update-metadata-'))
    try {
      const arm64 = writeUpdateManifest(directory, 'a-arm64', 'WorkDaddy-0.2.0-arm64.zip', 'arm64-digest', '2026-01-01T00:00:00.000Z')
      const x64 = writeUpdateManifest(directory, 'z-x64', 'WorkDaddy-0.2.0-x64.zip', 'x64-digest', '2026-02-02T00:00:00.000Z')
      const merged = mergeUpdateMetadata([arm64, x64], '0.2.0')
      expect(merged.path).toBe('WorkDaddy-0.2.0-x64.zip')
      expect(merged.sha512).toBe('x64-digest')
      expect(merged.releaseDate).toBe('2026-02-02T00:00:00.000Z')
      expect(merged.files.map((file: { url: string }) => file.url)).toEqual(['WorkDaddy-0.2.0-arm64.zip', 'WorkDaddy-0.2.0-x64.zip'])
      // Renaming the artifact directories reverses the path sort order without
      // changing which manifest supplies the legacy fields.
      const swappedArm64 = writeUpdateManifest(directory, 'z-arm', 'WorkDaddy-0.2.0-arm64.zip', 'arm64-digest', '2026-01-01T00:00:00.000Z')
      const swappedX64 = writeUpdateManifest(directory, 'a-intel', 'WorkDaddy-0.2.0-x64.zip', 'x64-digest', '2026-02-02T00:00:00.000Z')
  ...
  test('rejects updater metadata without a usable size or digest', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-update-metadata-invalid-'))
    try {
      ...
      const sized = (size: string) => `  - url: WorkDaddy-0.2.0-x64.zip\n    sha512: digest\n    size: ${size}\n`
      expect(() => mergeUpdateMetadata([write('missing-size.yml', '  - url: WorkDaddy-0.2.0-x64.zip\n    sha512: digest\n')], '0.2.0')).toThrow(/invalid size/)
      ...
      expect(() => mergeUpdateMetadata([write('empty-digest.yml', "  - url: WorkDaddy-0.2.0-x64.zip\n    sha512: ''\n    size: 42\n")], '0.2.0')).toThrow(/no sha512/)
  ...
  test('deletes only unexpected assets from a resumed draft release', () => {
    const assets = ['WorkDaddy-0.2.0-m-chip.dmg', 'WorkDaddy-0.1.9-arm64.dmg', 'latest-mac.yml', 'SHA256SUMS.txt']
    const expected = ['WorkDaddy-0.2.0-m-chip.dmg', 'latest-mac.yml', 'SHA256SUMS.txt']
    expect(staleDraftReleaseAssets(assets, expected)).toEqual(['WorkDaddy-0.1.9-arm64.dmg'])
  ...
  test('prints the stale draft assets when run as a script', () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-draft-prune-'))
    try {
      mkdirSync(join(directory, 'release-assets'))
      for (const name of ['WorkDaddy-0.2.0-m-chip.dmg', 'SHA256SUMS.txt']) writeFileSync(join(directory, 'release-assets', name), 'asset')
      writeFileSync(join(directory, 'draft.json'), JSON.stringify({ assets: [{ name: 'WorkDaddy-0.2.0-m-chip.dmg' }, { name: 'WorkDaddy-0.1.9-arm64.dmg' }] }))
      ...
      expect(result.stdout.trim().split('\n')).toEqual(['WorkDaddy-0.1.9-arm64.dmg'])
  ...
  test('selects the exact cross-platform asset set and writes reproducible checksums', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-github-release-'))
    const projectDirectory = join(directory, 'project')
    const inputDirectory = join(directory, 'downloaded')
    ...
    try {
      ...
      expect(macFeed).toContain('WorkDaddy-0.2.0-arm64.zip')
      expect(macFeed).toContain('WorkDaddy-0.2.0-x64.zip')
      expect(macFeed).toContain('WorkDaddy-0.2.0-m-chip.dmg')
      expect(macFeed).not.toContain('WorkDaddy-0.2.0-arm64.dmg')
      const linuxFeed = readFileSync(join(outputDirectory, 'latest-linux.yml'), 'utf8')
      expect(linuxFeed).toContain('WorkDaddy-0.2.0-linux-x86_64.AppImage')
      expect(linuxFeed).not.toContain('WorkDaddy-0.2.0-linux-arm64.AppImage')
      const linuxArmFeed = readFileSync(join(outputDirectory, 'latest-linux-arm64.yml'), 'utf8')
      expect(linuxArmFeed).toContain('WorkDaddy-0.2.0-linux-arm64.AppImage')
      expect(linuxArmFeed).not.toContain('WorkDaddy-0.2.0-linux-x86_64.AppImage')
    } finally {
      rmSync(directory, { recursive: true, force: true })
    }
  })
  ...
  test('rejects incomplete and duplicate downloaded release assets', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'workdaddy-github-release-invalid-'))
    const projectDirectory = join(directory, 'project')
    const inputDirectory = join(directory, 'downloaded')
    mkdirSync(projectDirectory, { recursive: true })
    mkdirSync(inputDirectory, { recursive: true })
...
describe('post-package verification helpers', () => {
  ...
  test('keeps every platform native unpack allowlist exact and architecture-specific', () => {
    ...
    expect(packageJson.author).toEqual({ name: 'WorkDaddy contributors' })
    expect(packageJson.description).toBe('A skill-first desktop workspace for oh-my-pi (OMP)')
    expect(packageJson.homepage).toBe('https://github.com/MQL9011/WorkDaddy')
    expect(packageJson.build.productName).toBe('WorkDaddy')
    expect(packageJson.build.appId).toBe('ai.maerwen.work')
    expect(packageJson.desktopName).toBe('workdaddy.desktop')
    ...
    // its full-size layer to WorkDaddy's canonical PNG and retain small layers.
  ...
  test('fails closed when Windows Authenticode verification is not valid', () => {
    const signerEnvironment = { WORKDADDY_WINDOWS_CERT_SUBJECT: 'CN=Example Ltd' }
    ...
    const validSpawn = (file: string, args: string[], options: { env?: NodeJS.ProcessEnv }) => {
      calls.push({ file, args, path: options.env?.WORKDADDY_SIGNED_FILE })
      return { status: 0, stdout: '', stderr: '' }
    }
    const spawn = validSpawn as unknown as Parameters<typeof assertValidAuthenticode>[1]
    expect(() => assertValidAuthenticode('C:\\release\\WorkDaddy.exe', spawn, signerEnvironment)).not.toThrow()
    expect(calls).toEqual([
      {
        file: 'powershell.exe',
        args: expect.arrayContaining(['-NoProfile', '-NonInteractive', '-Command']),
        path: 'C:\\release\\WorkDaddy.exe',
      },
    ])
    const invalidSpawn = () => ({ status: 1, stdout: '', stderr: 'NotSigned' })
    expect(() => assertValidAuthenticode('C:\\release\\WorkDaddy.exe', invalidSpawn as unknown as Parameters<typeof assertValidAuthenticode>[1], signerEnvironment)).toThrow(/NotSigned/)
  })

  test('asserts the expected Authenticode signer instead of any trusted certificate', () => {
    expect(expectedAuthenticodeSigner({ WORKDADDY_WINDOWS_CERT_SUBJECT: '  CN=Example Ltd, C=GB  ' })).toEqual({ subject: 'CN=Example Ltd, C=GB', thumbprint: '' })
    expect(expectedAuthenticodeSigner({ WORKDADDY_WINDOWS_CERT_THUMBPRINT: 'a1:b2 c3d4e5f60718293a4b5c6d7e8f901a2b3c4d' })).toEqual({
      subject: '',
      thumbprint: 'A1B2C3D4E5F60718293A4B5C6D7E8F901A2B3C4D',
    })
    expect(() => expectedAuthenticodeSigner({ WORKDADDY_WINDOWS_CERT_THUMBPRINT: 'not-a-thumbprint' })).toThrow(/40-character SHA-1 certificate thumbprint/)
    // Unconfigured fails closed rather than accepting whoever the runner trusts.
    expect(() => expectedAuthenticodeSigner({})).toThrow(/WORKDADDY_WINDOWS_CERT_SUBJECT and\/or WORKDADDY_WINDOWS_CERT_THUMBPRINT/)
    ...
    assertValidAuthenticode('C:\\release\\WorkDaddy.exe', spawn as unknown as Parameters<typeof assertValidAuthenticode>[1], {
      WORKDADDY_WINDOWS_CERT_SUBJECT: 'CN=Example Ltd',
      WORKDADDY_WINDOWS_CERT_THUMBPRINT: 'a1b2c3d4e5f60718293a4b5c6d7e8f901a2b3c4d',
    })
    ...
    expect(calls[0].env).toMatchObject({
      WORKDADDY_SIGNED_FILE: 'C:\\release\\WorkDaddy.exe',
      WORKDADDY_WINDOWS_CERT_SUBJECT: 'CN=Example Ltd',
      WORKDADDY_WINDOWS_CERT_THUMBPRINT: 'A1B2C3D4E5F60718293A4B5C6D7E8F901A2B3C4D',
    })
    const script = calls[0].args.at(-1) as string
    expect(script).not.toContain('C:\\release\\WorkDaddy.exe')
    expect(script).toContain("$ErrorActionPreference = 'Stop'")
    expect(script).toContain('try {')
    expect(script).toContain('} catch {')
    expect(script).toContain('exit 1')
    expect(script).toContain('$certificate.Subject -ne $env:WORKDADDY_WINDOWS_CERT_SUBJECT')
    expect(script).toContain('$certificate.Thumbprint.ToUpperInvariant() -ne $env:WORKDADDY_WINDOWS_CERT_THUMBPRINT')
  })
  ...
  test('checks every allowed native fixture file against the application architecture', () => {
    ...
    const directory = createUnpackedFixture(architectures)
    try {
      expect(() => assertUnpackedNativeLayout(directory, architectures, (path: string) => (path.endsWith('pty.node') ? new Set(['x86_64']) : new Set(['arm64'])))).toThrow(
        /pty\.node is missing app architecture.*arm64/,
      )
    } finally {
      rmSync(directory, { recursive: true, force: true })
    }
...
describe('cross-platform packaging repair', () => {
  ...
  test('the verifier and package.json agree on required native files', () => {
    const packageJson = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'))
