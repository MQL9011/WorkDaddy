...
describe('automatic update service', () => {
  it('uses explicit system messages for manual update checks', () => {
    expect(manualUpdateNotification({ phase: 'not-available' })).toMatchObject({ type: 'info', message: 'No WorkDaddy Update Available' })
    expect(manualUpdateNotification({ phase: 'available', version: '0.2.0' })).toMatchObject({ type: 'info', message: 'WorkDaddy Update Available' })
    expect(manualUpdateNotification({ phase: 'downloaded', version: '0.2.0' })).toMatchObject({ type: 'info', message: 'WorkDaddy Update Available' })
  })
  ...
  it('explains that builds without an update service cannot check', () => {
    expect(manualUpdateNotification({ phase: 'unsupported', message: 'Automatic updates are available in installed builds.' })).toEqual({
      type: 'info',
      message: 'WorkDaddy Updates Unavailable',
  ...
  it('coalesces concurrent manual update checks into one dialog', async () => {
    ...
    expect(notify).toHaveBeenCalledWith({ type: 'info', message: 'No WorkDaddy Update Available' })
  ...
  it('reports that development builds cannot check for updates', () => {
    ...
    expect(manualUpdateNotification(service.getState())).toMatchObject({ type: 'info', message: 'WorkDaddy Updates Unavailable' })
  })
})
