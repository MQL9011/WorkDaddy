...
import { boundedSessionDiscoveryNames, LIVE_CATALOG_MIN_SPAWN_INTERVAL_MS, SessionMetadataCatalog, type SessionCatalogIo } from '../../electron/main/sessions/catalog'
...
`)
    ...
    // The throttle compares real Date.now() timestamps against the burst below;
    // faking only Date keeps that comparison deterministic regardless of how
    // long the real CLI spawns and file I/O in this test actually take under
    // system load, while leaving those real async operations untouched.
    ...
    try {
      ...
      vi.advanceTimersByTime(LIVE_CATALOG_MIN_SPAWN_INTERVAL_MS + 1)
      catalog.invalidateLiveCatalog()
      await catalog.all()
...
readline.createInterface({ input: process.stdin }).on('line', (line) => {
  ...
  if (command.type === 'negotiate_protocol') {
    send({ id: command.id, type: 'response', command: 'negotiate_protocol', success: true })
