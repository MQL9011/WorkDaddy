...
describe('application window lifecycle', () => {
  it('gives migration failures a dedicated actionable startup dialog', () => {
    const migration = new StateMigrationError(
      ...
      'C:\\Users\\alice\\WorkDaddy\\prime-work-state-v4.json',
      'C:\\Users\\alice\\WorkDaddy\\prime-work-state.json',
      'C:\\Users\\alice\\WorkDaddy\\prime-work-state.json.migrated-v4-backup',
    )

    expect(startupFailureDialog(migration)).toMatchObject({
      title: 'WorkDaddy state migration failed',
  ...
  it('resolves packaged renderer assets with Windows path separators', () => {
    const root = 'C:\\WorkDaddy\\resources\\app.asar\\out\\renderer'
  ...
  it('asks nothing when no agent run is active', () => {
    const idle: RuntimeInfo = { runtimeId: 'a', harness: 'omp', cwd: '/tmp', isStreaming: false }

    expect(activeShutdownWork([idle])).toEqual({ runningAgents: 0 })
    expect(shutdownPrompt(activeShutdownWork([idle]))).toBeNull()
    expect(shutdownPrompt(activeShutdownWork([]))).toBeNull()
  })

  it('names the work at risk for streaming, compacting, and queued state', () => {
    const base = { harness: 'omp' as const, cwd: '/tmp', isStreaming: false }
    ...
    expect(activeShutdownWork(runtimes)).toEqual({ runningAgents: 4 })
    expect(shutdownPrompt({ runningAgents: 4 })).toEqual({
      message: 'Close WorkDaddy while an agent is running?',
      detail: '4 agent runs are still in progress and will be stopped.',
    })
    expect(shutdownPrompt({ runningAgents: 1 })).toEqual({
      message: 'Close WorkDaddy while an agent is running?',
      detail: 'An agent run is still in progress and will be stopped.',
    })
    expect(shutdownPrompt({ runningAgents: 0 })).toBeNull()
  })

  it('defaults to cancel and uses the non-blocking dialog', async () => {
    const window = { isDestroyed: () => false } as unknown as BrowserWindow
    const prompt = { message: 'Close WorkDaddy while an agent is running?', detail: 'An agent run is still in progress and will be stopped.' }
