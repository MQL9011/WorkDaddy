...
async function fixture(scope: { cwd: string; sessionPath?: string } = { cwd: '/project', sessionPath: '/sessions/one.jsonl' }) {
  ...
  const bridge = new TestAgentBrowserBridge({ service, terminals, extensionPath: '/app/extensions/omp-work-browser.ts' })
  await bridge.start()
  bridges.push(bridge)
...
describe('AgentBrowserBridge', () => {
  ...
  it('exposes the extension path and dispatches scoped methods', async () => {
  ...
  it('propagates service failures as bounded errors', async () => {
    ...
    const bridge = new AgentBrowserBridge({ service, terminals: fakeTerminals(), extensionPath: '/x.ts' })
    await bridge.start()
    bridges.push(bridge)
