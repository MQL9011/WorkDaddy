...
describe('application update menu', () => {
  it('puts Check for Updates in the app menu on macOS', () => {
    ...
    const template = buildApplicationMenuTemplate({ platform: 'darwin', appName: 'WorkDaddy', checkForUpdates, closeWindow: vi.fn() })
    const appMenu = template[0]
    const updateItem = submenu(appMenu).find((item) => item.label === 'Check for Updates…')

    expect(appMenu?.label).toBe('WorkDaddy')
  ...
  it('puts Check for Updates in Help on Windows and Linux', () => {
    for (const platform of ['win32', 'linux'] as const) {
      const checkForUpdates = vi.fn()
      const template = buildApplicationMenuTemplate({ platform, appName: 'WorkDaddy', checkForUpdates, closeWindow: vi.fn() })
  ...
  it('disables Check for Updates when the build has no update service', () => {
    for (const platform of ['darwin', 'win32', 'linux'] as const) {
      const template = buildApplicationMenuTemplate({ platform, appName: 'WorkDaddy', updatesEnabled: false, checkForUpdates: vi.fn(), closeWindow: vi.fn() })
    ...
    const enabled = buildApplicationMenuTemplate({ platform: 'linux', appName: 'WorkDaddy', checkForUpdates: vi.fn(), closeWindow: vi.fn() })
  ...
  it('installs the native application menu', () => {
    const checkForUpdates = vi.fn()
    installApplicationMenu({ platform: 'linux', appName: 'WorkDaddy', checkForUpdates, closeWindow: vi.fn() })
  ...
  it('routes macOS Command-Q through the main-window close path', () => {
    ...
    const template = buildApplicationMenuTemplate({ platform: 'darwin', appName: 'WorkDaddy', checkForUpdates: vi.fn(), closeWindow })
    const quitItem = submenu(template[0]).find((item) => item.label === 'Quit WorkDaddy')
