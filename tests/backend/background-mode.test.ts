...
describe('macOS background startup', () => {
  it('recognizes login launches and the explicit flag only on macOS', () => {
    expect(shouldStartInBackground(['/Applications/WorkDaddy', '--background'], false, 'darwin')).toBe(true)
    expect(shouldStartInBackground(['/Applications/WorkDaddy'], true, 'darwin')).toBe(true)
    expect(shouldStartInBackground(['/Applications/WorkDaddy'], false, 'darwin')).toBe(false)
    expect(shouldStartInBackground(['WorkDaddy.exe', '--background'], true, 'win32')).toBe(false)
  })
...
describe('MacBackgroundController', () => {
  it('keeps the process in the menu bar on close and exposes Open, Settings, and Quit', () => {
    ...
    expect(electron.templates[0].map((item) => item.label ?? item.type)).toEqual(['Open WorkDaddy', 'separator', 'Settings...', 'Quit'])
