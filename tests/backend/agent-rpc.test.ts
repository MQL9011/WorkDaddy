...
import { AgentRpcManager, OMP_RPC_ADAPTER, type ProviderCatalog } from '../../electron/main/agent-rpc'
...
input.on('line', (line) => {
  ...
  if (command.type === 'negotiate_protocol') { send({ id: command.id, type: 'response', command: 'negotiate_protocol', success: true }); return }
  ...
  it.each([
    ...
    try {
      for (const command of [
        ...
        await expect(manager.command('runtime-auth-policy', command)).rejects.toThrow('Network MCP authentication is managed outside WorkDaddy')
      }
...
describe('agent RPC responses', () => {
  it('uses a newly discovered executable for future runtime starts', async () => {
    ...
    await expect(manager.start({ cwd: fake.cwd })).rejects.toThrow('OMP executable was not found')
    executable = fake.executable
    await expect(manager.start({ cwd: fake.cwd })).resolves.toMatchObject({ harness: 'omp', cwd: fake.cwd })
  })
...
`)
  ...
  it('decorates runtime reasoning and fast-mode capabilities from the model catalog', async () => {
    ...
    const providers = {
      capabilities: async () => ({
        key: 'openai-codex/gpt-5.4', provider: 'openai-codex', id: 'gpt-5.4', name: 'GPT-5.4', reasoning: true,
        input: ['text'], contextWindow: 200_000, maxTokens: 32_000, availableThinkingLevels: ['off', 'low', 'medium', 'high', 'xhigh'], fastModeSupported: true, available: true,
      }),
...
readline.createInterface({ input: process.stdin }).on('line', (line) => {
  const command = JSON.parse(line)
  if (command.type === 'negotiate_protocol') { process.stdout.write(JSON.stringify({ id: command.id, type: 'response', command: 'negotiate_protocol', success: true }) + '\\n'); return }
  if (command.type === 'get_state') {
