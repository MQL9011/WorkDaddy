...
function ReasoningPart({ part, streaming = false }: { part: Extract<MessagePart, { type: 'thinking' }>; streaming?: boolean }) {
  const { t } = useI18n()
  return <div className="activity-line activity-line--reasoning"><MarkdownText text={boundText(part.text, 40_000, t('transcript.truncated.reasoning'))} streaming={streaming} /></div>
}
  ...
  return (
    <div className={`activity-line activity-line--tool activity-line--${kind} is-${state}`}>
      <button type="button" className="activity-tool__summary" disabled={!canExpand} onClick={() => setOpen((value) => !value)} aria-expanded={canExpand ? open : undefined}>
        ...
        <span className="activity-line__kind">{kind === 'question' ? t('transcript.tool.question') : part.name}</span>
        {preview ? <code className="activity-tool__preview"><SyntaxText text={preview} /></code> : null}
        <span className="activity-tool__state">{failed ? <><CircleAlert size={12} /> {t('transcript.tool.state.failed')}</> : finished ? <><Check size={12} /> {t('transcript.tool.state.done')}</> : kind === 'question' ? t('transcript.tool.state.needsInput') : <><LoaderCircle className="spin" size={12} /> {t('transcript.tool.state.running')}</>}</span>
      ...
      {open && visibleOutput ? (
        <div className="activity-tool__details">
          <div className="activity-tool__details-toolbar">
            <button type="button" className="activity-tool__copy" aria-label={t('transcript.copyToolContentsAria', { action: copied ? t('common.copied') : t('common.copy') })} onClick={() => void copyContents()}>
...
function AgentActivityPart({ part }: { part: Extract<MessagePart, { type: 'agentMessage' }> }) {
  ...
  return <div className="activity-line activity-line--agent">
    <button type="button" className="activity-tool__summary" disabled={!canExpand} onClick={() => setOpen((value) => !value)} aria-expanded={canExpand ? open : undefined}>
      ...
      <span className="activity-line__kind">{t('transcript.messageFromAgent')}</span>
      {part.agentName ? <span className="activity-agent__name">{part.agentName}</span> : null}
      {canExpand ? open ? <ChevronDown size={13} /> : <ChevronRight size={13} /> : null}
    </button>
    {open && part.text ? <div className="activity-agent__details"><MarkdownText text={boundText(part.text, 40_000, t('transcript.truncated.agentMessage'))} /></div> : null}
  </div>
}

function compactionReasonLabel(reason: Extract<MessagePart, { type: 'compaction' }>['reason'], t: TFunction): string {
  if (reason === 'threshold') return t('transcript.compaction.reason.threshold')
  if (reason === 'overflow') return t('transcript.compaction.reason.overflow')
  if (reason === 'requested') return t('transcript.compaction.reason.requested')
  if (reason === 'manual') return t('transcript.compaction.reason.manual')
  return t('transcript.compaction.reason.context')
}
...
function CompactionPart({ part }: { part: Extract<MessagePart, { type: 'compaction' }> }) {
  ...
  const label = part.status === 'running'
    ? t('transcript.compaction.status.running')
    : part.status === 'done'
      ? t('transcript.compaction.status.done')
      : part.status === 'cancelled' ? t('transcript.compaction.status.cancelled') : part.outcome === 'skipped' ? t('transcript.compaction.status.skipped') : t('transcript.compaction.status.failed')
  const state = part.status === 'running'
    ? <><LoaderCircle className="spin" size={12} /> {t('transcript.tool.state.running')}</>
    : part.status === 'done'
      ? <><Check size={12} /> {part.willRetry ? t('transcript.compaction.continuing') : t('transcript.tool.state.done')}</>
  ...
  return <div className={`activity-line activity-line--compaction is-${part.status}`}>
    <button type="button" className="activity-tool__summary" disabled={!canExpand} onClick={() => setOpen((value) => !value)} aria-expanded={canExpand ? open : undefined}>
      ...
      <span className="activity-compaction__reason">{compactionReasonLabel(part.reason, t)}</span>
      {tokenLabel ? <span className="activity-compaction__tokens">{tokenLabel}</span> : null}
      <span className="activity-tool__state">{state}</span>
      {canExpand ? open ? <ChevronDown size={13} /> : <ChevronRight size={13} /> : null}
    </button>
    {open && details ? <div className={`activity-compaction__details ${part.status === 'done' ? '' : 'is-error'}`}><MarkdownText text={boundText(details, 40_000, t('transcript.truncated.compaction'))} /></div> : null}
  </div>
}
...
export function WorkDisclosure({ message, parts, showReasoning, showTools, running = message.streaming }: { message: TranscriptMessage; parts: MessagePart[]; showReasoning: boolean; showTools: boolean; running?: boolean }) {
  ...
  if (running) {
    ...
    return <section className="work-disclosure is-running" aria-label={t('transcript.workActivityAria')}>
      <span className="work-disclosure__rail" aria-hidden="true" />
      <div className="work-disclosure__live">
        <div className="work-disclosure__status" role="status"><span>{t(status === 'Thinking' ? 'transcript.status.thinking' : 'transcript.status.working')}</span><ThinkingDots /></div>
