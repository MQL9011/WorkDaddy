...
const TerminalView = forwardRef<TerminalViewHandle, TerminalViewProps>(function TerminalView({ cwd, sessionPath, shell, label, visible, onStateChange, onSelectionChange, onError }, ref) {
  ...
  useEffect(() => {
    ...
    const announceExit = (exitCode: number) => {
      terminal.writeln(`\r\n\x1b[90m${tRef.current('terminal.processExited', { code: exitCode })}\x1b[0m`)
    ...
    if (window.prime && cwd) {
      ...
      window.prime.terminal.create({ cwd, shell, cols: terminal.cols, rows: terminal.rows }).then(({ terminalId, shell: actualShell }) => {
        ...
        if (cancelled) return
        const message = error instanceof Error ? error.message : tRef.current('terminal.unableToStart')
        terminal.writeln(`\x1b[31m${message}\x1b[0m`)
        onErrorRef.current?.(message)
      })
    } else {
      terminal.writeln(`\x1b[38;5;141m${tRef.current('terminal.placeholder.title')}\x1b[0m`)
      terminal.writeln(`\x1b[90m${tRef.current('terminal.placeholder.description')}\x1b[0m`)
...
export const TerminalDrawer = forwardRef<TerminalDrawerHandle, TerminalDrawerProps>(function TerminalDrawer({ visible = true, cwd, sessionPath, shell, height, minHeight, maxHeight, defaultHeight, onHeightChange, onClose, onError, onSelectionChange }, ref) {
  ...
  const addTerminal = () => {
    if (tabs.length >= MAX_TERMINAL_TABS) {
      onError?.(t('terminal.maxTabsError', { count: MAX_TERMINAL_TABS }))
      return
    }
  ...
  return (
    <section className={`terminal-drawer ${maximized ? 'is-maximized' : ''}`} aria-label={t('terminal.aria.section')} hidden={!visible}>
      {!maximized ? <ResizeHandle orientation="horizontal" label={t('terminal.resize.label')} value={height} min={minHeight} max={maxHeight} defaultValue={defaultHeight} onChange={onHeightChange} /> : null}
