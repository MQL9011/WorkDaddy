...
const MarkdownLink: Components['a'] = ({ node: _node, href, children, ...props }) => {
  const { t } = useI18n()
  return href && (/^(https?:|mailto:|#)/i.test(href))
    ...
    : <span className="markdown-link-unsupported" title={href ? t('transcript.projectRelativeLink', { href }) : undefined}>{children}</span>
}
const MarkdownImage: Components['img'] = ({ alt }) => {
  const { t } = useI18n()
  return <span className="markdown-image-placeholder">{t('transcript.imagePlaceholder', { alt: alt || t('transcript.imagePlaceholderDefaultAlt') })}</span>
}
