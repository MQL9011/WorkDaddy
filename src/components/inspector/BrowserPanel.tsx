...
function agentTabLabel(tab: AgentBrowserTabRecord, t: (key: MessageKey, values?: Record<string, string | number>) => string): string {
  if (tab.title) return tab.title
  try {
    const host = new URL(tab.url).hostname
    if (host) return host
  } catch { /* about:blank and friends */ }
  return t('inspector.browser.newTab')
}
...
export function BrowserPanel({ home, onOpenExternal, annotations, agentTabs = [], activeAgentTabId = null, previewSelected = true, onSelectAgentTab, onCloseAgentTab, onShowPreview, onAgentSlotRect, agentSessionKey, onPreviewContext, previewPointerEvent = null, onNavigateAgentTab, pollIntervalMs = 350, platform = detectRendererPlatform() }: BrowserPanelProps) {
  ...
  const toggleAnnotation = () => {
    ...
    if (annotationsRef.current.atCapacity) {
      showNotice(t('inspector.browser.annotationLimitNotice', { max: MAX_BROWSER_ANNOTATIONS }))
      return
    }
    setNotice('')
    setPicking(true)
  ...
  return (
    <div className="browser-panel">
      {agentTabs.length ? (
        <div className="browser-tabstrip" role="tablist" aria-label={t('inspector.browser.tabsAria')}>
          <button type="button" role="tab" aria-selected={!showAgentTab} className={showAgentTab ? '' : 'is-active'} onClick={() => onShowPreview?.()}>
            {t('inspector.browser.previewTab')}
          </button>
          {agentTabs.map((tab) => (
            <div
              key={tab.tabId}
            ...
            >
              ...
              <span>{agentTabLabel(tab, t)}</span>
              <button
                type="button"
                aria-label={t('inspector.browser.closeAgentTab', { name: agentTabLabel(tab, t) })}
                onClick={(event) => {
      ...
      <div className="browser-toolbar">
        ...
        >
          <ShieldCheck size={12} />
          <input value={address} onChange={(event) => setAddress(event.target.value)} aria-label={t('inspector.browser.addressAria')} spellCheck={false} />
          <button type="button" aria-label={t('inspector.browser.historyAria')} onClick={() => setHistoryOpen((value) => !value)}>
            <History size={13} />
          </button>
        </form>
        <IconButton className={picking || pendingElement ? 'is-active annotation-active' : ''} label={picking ? t('inspector.browser.stopAnnotating') : t('inspector.browser.annotatePage')} aria-pressed={picking} onClick={toggleAnnotation}>
      ...
      <div className={`browser-viewport ${picking ? 'is-annotating' : ''}`}>
        ...
        {picking ? (
          <div className="annotation-hint" role="status">
            <MessageCirclePlus size={12} /> {t('inspector.browser.annotationHint')}
          </div>
        ) : null}
        {pendingElement ? (
          <div className="annotation-layer">
            <div className="annotation-popover">
              <div>
                ...
                <strong>{t('inspector.browser.commentOnElement', { number: count + 1 })}</strong>
                <button
                  type="button"
                  aria-label={t('inspector.browser.discardAnnotation')}
                  onClick={() => {
                    setPendingElement(null)
                    setAnnotationText('')
                  }}
              ...
              <textarea
                ...
                placeholder={t('inspector.browser.annotationPlaceholder')}
              />
        ...
        {count ? (
          <div className="annotation-count">
            ...
            {staleCount ? ` · ${t('inspector.browser.staleFromEarlier', { count: staleCount })}` : ''}
          </div>
        ) : null}
        {notice ? (
          ...
          <div className="browser-agent-urlbar">
            ...
            >
              ...
              <input
                ...
                aria-label={t('inspector.browser.agentAddressAria')}
                spellCheck={false}
              />
            </form>
            {!activeAgentTab.attached ? <em>{t('inspector.browser.connecting')}</em> : null}
          </div>
          <div className="browser-agent-slot" ref={slotRef} aria-label={t('inspector.browser.agentSlotAria')} />
        </div>
      ) : null}
    </div>
  )
