    ...
    <Modal title={request.title} onClose={cancel} footer={(
      ...
      {request.method === 'questionnaire' ? (
        <div ref={questionnaireRef} className="extension-questionnaire" onKeyDown={handleQuestionnaireKeyDown}>
          {!request.complete ? <p className="modal-intro">{t('extensionUi.preparingQuestions')}</p> : null}
          {request.complete ? (
            <>
              {remainingMs !== undefined ? (
                <div className="extension-questionnaire__timer" role="timer" aria-live="polite">
                  <span>{t('extensionUi.timeRemaining')}</span>
                  <strong aria-label={t('extensionUi.secondsRemaining', { seconds: Math.max(0, Math.ceil(remainingMs / 1_000)) })}>
                    {Math.max(0, Math.ceil(remainingMs / 1_000))}s
                  </strong>
                </div>
              ) : null}
              <div className="extension-questionnaire__progress" aria-label={t('extensionUi.questionProgressAria')}>
                ...
                <button type="button" aria-current={questionIndex === request.questions.length ? 'step' : undefined} className={questionIndex === request.questions.length ? 'is-active' : ''} onClick={() => navigateQuestion(request.questions.length)}>✓ {t('extensionUi.submitAnswers')}</button>
              </div>
              {activeQuestion ? (
                <div className="extension-questionnaire__question">
                  ...
                  <p className="extension-questionnaire__hint">{t('extensionUi.questionnaireHint', { prev: shortcutLabel(platform, ['Primary', 'ArrowLeft']), next: shortcutLabel(platform, ['Primary', 'ArrowRight']) })}</p>
                </div>
              ) : (
