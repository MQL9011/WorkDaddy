...
function routingInstruction(mention: CapabilityMention): string {
  ...
  if (mention.skill.id === 'prime-work-browser' || mention.skill.id === 'omp-work-browser') {
    return `${token}: use WorkDaddy's in-app Browser capability and its own browser tools. Do not substitute Chrome, an external browser, or another connected browser tool.`
  }
  if (mention.skill.id === 'prime-work-schedules' || mention.skill.id === 'omp-work-schedules') {
    return `${token}: use WorkDaddy's Scheduled tasks capability and its own schedule tools.`
  }
  return `${token}: use the enabled WorkDaddy ${mention.skill.kind} with the exact catalog id ${JSON.stringify(mention.skill.id)}. Do not substitute a similarly named capability.`
}
