...
export function appendSessionRouting(prompt: string, sessions: readonly SessionRecord[], preferredIds: ReadonlyMap<string, string> = new Map()): string {
  ...
  const references = unique.map(({ text, session }) => `- ${JSON.stringify(text)}: ${session.harness} session UUID ${session.id}. Use session_read, session_send, and session_wait when the request requires collaboration; WorkDaddy can wake an idle saved target.`)
  return `${visiblePrompt}\n\n${SESSION_ROUTING_BEGIN}\nThe user explicitly referenced these WorkDaddy sessions. Titles are display labels; use the exact UUIDs below:\n${references.join('\n')}\n${SESSION_ROUTING_END}`
}
