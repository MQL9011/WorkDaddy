...
export async function openExternalUrl(app: ShellApi, url: string): Promise<string | null> {
  try {
    return await app.openExternal(url) ? null : `WorkDaddy could not open ${url} in your browser.`
  } catch (error) {
...
export async function revealPath(app: ShellApi, path: string): Promise<string | null> {
  try {
    return await app.revealPath(path) ? null : `WorkDaddy could not reveal ${path} in your file manager.`
  } catch (error) {
    return errorMessage(error)
  }
}
