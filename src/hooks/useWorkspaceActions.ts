...
import type { CapabilityMutationInput, ExtensionInstallInput, GitStatus, HarnessId, McpConnectionInput, McpStateInput, PrimeWorkApi, ProjectRecord, PromptDeliveryIntent, PromptImage, SessionRecord, TranscriptMessage, WorkspaceView } from '@/types/api'
...
export type McpCommand = { type: 'authenticate'; server?: string }
...
export function createWorkspaceActions(getDeps: () => WorkspaceActionsDeps) {
  ...
  const selectSession = async (session: SessionRecord) => {
    const { layout, settingsState, projects, workspace, setSessions, setView, clearSessionAttention, reportError, t } = getDeps()
    if (layout.compactLayout) { layout.setSmallestSidebarAllowed(false); settingsState.setSidebarOpen(false) }
    clearSessionAttention(session)
    setSessions((items) => items.map((item) => item.id === session.id ? { ...item, unread: false } : item))
    const project = findProjectForSession(projects, session)
    if (!project) { reportError(t('toast.sessionNotInProject')); return }
    const generation = workspace.activateWorkspace(project, session)
    setView('session')
  ...
  const renameSession = async (session: SessionRecord, title: string) => {
    const { bridge, settingsState, setSessions, setToast, reportError, t } = getDeps()
    if (!bridge) return
    try {
      if (!await bridge.sessions.rename(session.filePath, title)) throw new Error(t('toast.sessionRenameFailed', { agent: HARNESS_AGENT_NAMES[settingsState.settings.activeHarness] }))
      setSessions((items) => items.map((item) => item.id === session.id ? { ...item, title } : item)); setToast(t('toast.sessionRenamed'))
  ...
  const removeProject = async (project: ProjectRecord) => {
    const { bridge, projects, sessions, workspace, setProjects, setToast, reportError, t } = getDeps()
    try {
      if (bridge && !await bridge.projects.remove(project.id, project.harness)) throw new Error(t('toast.projectRemoveFailed'))
      ...
      setToast(t('toast.projectRemoved'))
    } catch (error) { reportError(error) }
  }

  const sendPrompt = async (prompt: string, images: PromptImage[] = [], intent: PromptDeliveryIntent = 'queue') => {
    const { bridge, sessions, workspace, provider, settingsState, submissionAdmissionRef, demoTimerRef, setSessions, setSubmitting, setToast, reportError, t } = getDeps()
    ...
    if (mcpCommand?.type === 'authenticate') {
      const agent = HARNESS_AGENT_NAMES[commandHarness]
      setToast(mcpCommand.server ? t('toast.mcpAuthWithServer', { agent, server: mcpCommand.server }) : t('toast.mcpAuthGeneral', { agent }))
      return
    }
        ...
        try {
          ...
          if (workspace.workspaceRef.current.generation === currentWorkspace.generation) {
            ...
            workspace.setMessages((items) => [
              ...items,
              { id: `error-${Date.now()}`, role: 'system', timestamp: Date.now(), parts: [{ type: 'text', text: t('workspace.steerNotDelivered', { detail: requestFailureMessage(error) }) }] },
            ])
          }
          reportError(error)
          throw error
    ...
    const admittedSubmission = await submissionAdmissionRef.current.run(async () => {
      ...
      try {
        if (!admitted.project || !admitted.cwd) { reportError(t('toast.addProjectBeforeSession')); return }
        ...
        if (images.length > 0 && provider.model !== 'auto' && !provider.selectedModel?.input.includes('image')) {
          reportError(t('toast.modelNoImageSupport'))
          return
        }
        ...
        if (!bridge) {
          ...
          workspace.setMessages((items) => [...items, { id: assistantId, role: 'assistant', timestamp: Date.now(), startedAt: Date.now(), streaming: true, parts: [{ type: 'thinking', text: t('workspace.demo.reviewing') }] }])
          demoTimerRef.current.push(window.setTimeout(() => workspace.setMessages((items) => items.map((item) => item.id === assistantId ? { ...item, parts: [...item.parts, { type: 'toolCall', id: 'demo-tool', name: t('workspace.demo.toolName'), args: { cwd: admitted.cwd } }] } : item)), 450))
          demoTimerRef.current.push(window.setTimeout(() => workspace.setMessages((items) => items.map((item) => item.id === assistantId ? { ...item, streaming: false, completedAt: Date.now(), parts: [...item.parts, { type: 'toolResult', name: t('workspace.demo.toolName'), text: t('workspace.demo.toolResult') }, { type: 'text', text: t('workspace.demo.response') }] } : item)), 1_250))
          return
        }
        await grantProject(admitted.project)
        if (workspace.workspaceRef.current.generation !== generation) return
        const selected = workspace.workspaceRef.current
        if (!selected.cwd) throw new Error(t('workspace.noWorkingDirectory'))
        ...
        if (!activeRuntime) {
          ...
          if (images.length > 0 && selected.sessionFile && selectedSession?.status === 'running') {
            throw new Error(t('workspace.imagesQueueBlocked'))
          }
        ...
        if (activeRuntime.cwd !== selected.cwd || (selected.sessionFile && activeRuntime.sessionFile !== selected.sessionFile)) {
          ...
          throw new Error(t('workspace.runtimeMismatch', { agent: HARNESS_AGENT_NAMES[activeHarness] }))
        }
    ...
    if (!admittedSubmission) {
      const error = new Error(t('toast.submissionBusy'))
      reportError(error)
      throw error
    }
  }
  ...
  const installSkill = async (source: string) => {
    const { bridge, reportError, settingsState, t } = getDeps()
    if (!bridge) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.packageInstallDesktopOnly') }
  ...
  const installExtension = async (input: ExtensionInstallInput) => {
    const { activeProject, bridge, pluginSkills, reportError, settingsState, t } = getDeps()
    if (!bridge) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.extensionInstallDesktopOnly') }
    try {
      let installation = input
      if (input.scope === 'project') {
        if (!activeProject) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.openProjectBeforeExtension') }
  ...
  const connectMcp = async (input: McpConnectionInput) => {
    const { bridge, activeProject, pluginSkills, reportError, settingsState, t } = getDeps()
    if (!bridge) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.mcpConnectionsDesktopOnly') }
    try {
      let connection = input
      if (input.scope === 'project') {
        if (!activeProject) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.openProjectBeforeMcp') }
  ...
  const setMcpEnabled = async (input: McpStateInput) => {
    ...
    try {
      let update = input
      if (input.scope === 'project') {
        if (!activeProject) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.openProjectBeforeMcpChange') }
  ...
  const mutateCapability = async (input: CapabilityMutationInput) => {
    const { bridge, activeProject, pluginSkills, reportError, settingsState, t } = getDeps()
    if (!bridge) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.capabilitiesDesktopOnly') }
    try {
      let update = input
      if (input.scope === 'project') {
        if (!activeProject) return { ok: false as const, reason: 'blocked' as const, output: t('workspace.openProjectBeforeCapability') }
        const project = await grantProject(activeProject); update = { ...input, projectPath: project.primaryFolder }
      }
